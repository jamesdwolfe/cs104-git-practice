...
Order& Order::operator=(const Order &rhs)
{
	...
	clearOrder();
	Order tempRhs(rhs);
	for (size_t i = 0; i < tempRhs.mItems.size(); ++i){
	    mItems.push_back(new Item(*tempRhs.mItems[i]));
	}
...
Order Order::operator+(const Order &rhs) const
{
	...
	for (size_t i = 0; i < tempRhs.mItems.size(); ++i){
		...
		for(size_t j = 0; j < result.mItems.size(); ++j){
			if (tempRhs.mItems[i]->name == result.mItems[j]->name){
				result.mItems[j]->count++;
				matchFound = true;
				break;
			}
		}
		if(!matchFound) {
	    	result.mItems.push_back(new Item(*tempRhs.mItems[i]));
	    }
	}
